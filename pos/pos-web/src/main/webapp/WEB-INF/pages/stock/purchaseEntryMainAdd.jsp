...
$(document).ready(function () {
    ...
    //加载明细
    if('${id}' != '0'){
        $.post('${ctx}/stock/purchaseEntryMain/getEdit',{id:${id}},function(data){
            url = "${ctx}/stock/purchaseEntryMain/update";
            $('#id').val(data.id);
            ...
            edit = false;
            //明细
            $('#grid').datagrid('loadData', { total: data.purchaseEntryDetailList.length, rows: data.purchaseEntryDetailList });
            //禁用
            disable();
            //状态
...
function disable(){
    ...
    $('#purchaseEntryTypeCode').attr("disabled","disabled");
...
function modify(){
    ...
    $('#purchaseEntryTypeCode').removeAttr("disabled","disabled");
...
function save() {
    ...
    if(data.purchaseEntryTypeCode == null || data.purchaseEntryTypeCode == ""){
        alertLittle("请选择入库类型");
        return;
    }
    ...
    data.purchaseEntryDetailList = items;
