...
<script type="text/javascript">
var exportUrl = '${ctx}/export/purchaseReturnReport';
$(document).ready(function () {
    $("#grid").datagrid({
        url: '${ctx}/stock/purchaseReturnMain/getList',
        title: '采购退货单列表',
        selectOnCheck: true,
        pagination: true,
        rownumbers: true,
        ...
        columns: [
            [
                ...
                { field: 'purchaseReturnTypeName', title: '采购退货类型', fitColumns: true},
                { field: 'orgName', title: '店铺名称', fitColumns: true},
                { field: 'warehouseName', title: '采购退货店仓', fitColumns: true},
...
function audit() {
    ...
    $.post('${ctx}/stock/purchaseReturnMain/audit', {id: id}, function (data) {
...
function cancel() {
    ...
    $.post('${ctx}/stock/purchaseReturnMain/cancel', {id: id}, function (data) {
        alert("提示", data.msg, 2000);
        $('#grid').datagrid('reload');
    });
}
//跳到新增页面
function create() {
    window.location.href = "${ctx}/stock/purchaseReturnMain/add?id=" + 0;
}
//查看
function see() {
    var item = $('#grid').datagrid('getChecked');
    ...
    window.location.href = "${ctx}/stock/purchaseReturnMain/add?id=" + item[0].id
}
function reset(){
    $('#billNo').val("");
    $('#purchaseReturnTypeCode').val("");
...
function search() {
    ...
    var purchaseReturnTypeCode = $('#purchaseReturnTypeCode').val();
    ...
    $('#grid').datagrid('reload', {
        billNo: billNo,
        purchaseReturnTypeCode: purchaseReturnTypeCode,
    ...
    exportUrl = '${ctx}/export/purchaseReturnReport' +'?billNo='+billNo + '&purchaseReturnTypeCode='+purchaseReturnTypeCode + '&supplierInfoCode='+supplierInfoCode +'&orgCode=' + orgCode
